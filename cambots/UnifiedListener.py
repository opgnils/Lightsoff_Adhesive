...
def detect_serial_ports():
    ...
    for device in acm_devices:
        try:
            ...
            print(f"[UnifiedListener] {device} USB info: '{device_info}'")
            ...
            if "vesc" in device_info or "stm" in device_info or "vedder" in device_info:
                vesc_port = device
                print(f"[UnifiedListener] Identified {device} as VESC based on USB info")
            elif "arduino" in device_info or "usb serial" in device_info:
                arduino_port = device
                print(f"[UnifiedListener] Identified {device} as Arduino based on USB info")
    ...
    # If only one device, be conservative and assume it's the Arduino
        ...
        print(f"[UnifiedListener] Only one device found, assuming Arduino: {arduino_port}")
    
    # If we have more than 2 devices, try to identify or warn
    elif len(acm_devices) > 2:
        if arduino_port is None or vesc_port is None:
            print(f"[UnifiedListener] WARNING: Found {len(acm_devices)} ACM devices but could not confidently identify all!")
            print(f"[UnifiedListener] Arduino: {arduino_port}, VESC: {vesc_port}")
            print(f"[UnifiedListener] You may need to manually specify ports or improve detection logic")

    print(f"[UnifiedListener] Final assignment -> Arduino: {arduino_port}, VESC: {vesc_port}")
...
def open_arduino_serial(port):
    ...
    if port is None:
        print("[UnifiedListener] No Arduino serial port configured.")
        return None
    try:
...
def open_vesc_serial(port):
    ...
    if port is None:
        print("[UnifiedListener] No VESC serial port configured.")
        return None
    try:
...
def vesc_set_rpm(ser, rpm: int):
    ...
    try:
        ser.write(frame)
        ser.flush()
        print(f"[UnifiedListener] Sent to VESC: RPM={rpm}")
    except Exception as e:
        print(f"[UnifiedListener] Error writing to VESC serial: {e}")
        try:
            ser.close()
        except Exception:
            pass
    ...
    while not _vesc_thread_stop:
        ...
        if vesc_ser is None or not vesc_ser.is_open:
            if vesc_port is not None:
                print("[UnifiedListener] VESC keepalive: serial not open, attempting reopen...")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            if e.errno == 98:  # Address already in use
                print(f"[UnifiedListener] ERROR: Port {PORT} is already in use!")
                print(f"[UnifiedListener] Kill the existing process with: pkill -9 -f UnifiedListener")
                print(f"[UnifiedListener] Or check what's using the port: lsof -i :{PORT}")
                sys.exit(1)
            else:
                raise
        
        s.listen(5)
        print(f"[UnifiedListener] Listening for commands on {HOST}:{PORT}")

        try:
            while True:
                conn, addr = s.accept()
                with conn:
                    print(f"[UnifiedListener] Connection from {addr}")
                    # Read once per connection; commands are small
                    try:
                        data = conn.recv(1024)
                    except Exception as e:
                        print(f"[UnifiedListener] Error receiving TCP data: {e}")
                        continue
                    ...
                    try:
                        ...
                        print(f"[UnifiedListener] Decode error: {e}")
                        continue
                    ...
                    parts = cmd.split(",")
                    if len(parts) != 3:
                        print(f"[UnifiedListener] Ignoring malformed command: {cmd}")
                        continue
                    ...
                    try:
                        if arduino_ser is None or not arduino_ser.is_open:
                            if arduino_port is not None:
                                print("[UnifiedListener] Arduino serial not open, attempting reopen...")
                                arduino_ser = open_arduino_serial(arduino_port)
                                if arduino_ser is None:
                                    print("[UnifiedListener] Reopen of Arduino serial failed, dropping Arduino part of command.")
                                    continue
                            else:
                        ...
                        arduino_ser.flush()
                        print(f"[UnifiedListener] Sent to Arduino: {line.strip()}")
                    except Exception as e:
                        print(f"[UnifiedListener] Error writing to Arduino serial: {e}")
                        ...
                        arduino_ser = open_arduino_serial(arduino_port)
                        if arduino_ser is None:
                            print("[UnifiedListener] Could not recover Arduino serial; future Arduino commands may fail until process is restarted.")
        finally:
            print("[UnifiedListener] Shutting down, closing serials.")
